...
$resul = $con->query("SELECT OeId FROM opcre WHERE SubmissionId = (SELECT SubmissionId FROM submission WHERE UserId = '$test' AND (SubmissionStatus = 'Ongoing' OR SubmissionStatus = 'Submitted') AND QuarterId = (SELECT QuarterId FROM quarter WHERE QuarterProgress='Ongoing')) ORDER BY OeId DESC");
$resul2 = $con->query("SELECT OeId FROM opcre WHERE SubmissionId = (SELECT SubmissionId FROM submission WHERE UserId = '$test' AND (SubmissionStatus = 'Ongoing' OR SubmissionStatus = 'Submitted') AND QuarterId = (SELECT QuarterId FROM quarter WHERE QuarterProgress='Ongoing'))");
...
<div class="Form-Group1">
                    ...
                    <?php
                            ...
                            else
                            {
                                ...
                                else
                                {
                                    if($addstat == 1)
                                    {
                                        $pick = "SELECT OeId FROM opcre ORDER BY OeId DESC LIMIT 1";
    ...
    <form name="actorInsert" onSubmit="return ValidateActInsert()" action="Insert/OPCR2Insert.php" method="POST" enctype="multipart/form-data">
        <?php
            $output = null;
            $datefrom = null;
            $dateto = null;
            ...
            {
                $queryr = "SELECT OeId FROM opcre ORDER BY OeId DESC";
            ...
            $query = "SELECT * FROM opcre WHERE OeId = '$record'";
            ...
            if(mysqli_num_rows($query_run)>0)
            {
                ...
                {
                    $output = $row['OeOutput'];
                    $datefrom = $row['OeTargetDate'];
                    $dateto = $row['OeActualDate'];
                    $desc = $row['OeDesc'];
                    $status = $row['OeStatus'];
                    $remarks = $row['OeRemarks'];
                    $file = $row['OeTempName'];
                    $original = $row['OeFilename'];
                    $change = $row['OeId'];
                }
            }
        ...
        ?>
        <div class= "col-md-12">
            <label>Commitment Measurable by Efficiency</label>
        </div>
        <div class="form-group">
            <div class="col-md-11">
