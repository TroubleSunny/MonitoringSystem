...
$resul = $con->query("SELECT UtiId FROM researchutilization WHERE SubmissionId = (SELECT SubmissionId FROM submission WHERE UserId = '$test' AND (SubmissionStatus = 'Ongoing' OR SubmissionStatus = 'Submitted') AND QuarterId = (SELECT QuarterId FROM quarter WHERE QuarterProgress='Ongoing')) ORDER BY UtiId DESC");
$resul2 = $con->query("SELECT UtiId FROM researchutilization WHERE SubmissionId = (SELECT SubmissionId FROM submission WHERE UserId = '$test' AND (SubmissionStatus = 'Ongoing' OR SubmissionStatus = 'Submitted') AND QuarterId = (SELECT QuarterId FROM quarter WHERE QuarterProgress='Ongoing'))");
...
<section id="part-banner-specific">
    <div class="container">
        <div class="row">
            <div class="col-lg-12">
                <h4 class="pageTitle">A.5. Research Utilization</h4>
            </div>
        </div>
    </div>
</section>
...
<div class="Form-Group1">
   <form action="" method="GET">
                      ...
                      $recordcounter = mysqli_num_rows($resul);
                            ...
                            else
                            {
                                ...
                                else
                                {
                                    if($addstat == 1)
                                    {
                                        $pick = "SELECT UtiId FROM researchutilization ORDER BY UtiId DESC LIMIT 1";
    ...
    <form name="actorInsert" onSubmit="return ValidateActInsert()" action="Insert/ResearchUtilizationInsert.php" method="POST" enctype="multipart/form-data">
        <?php
            $class = null;
            $category = null;
            $agenda = null;
            ...
            {
                $queryr = "SELECT UtiId FROM researchutilization ORDER BY UtiId DESC";
            ...
            $query = "SELECT * FROM researchutilization WHERE UtiId = '$record'";
            ...
            if(mysqli_num_rows($query_run)>0)
            {
                foreach($query_run as $row)
                {
                    $title = $row['RuTitle'];
                    $class =  $row['RuClass'];
                    $category =  $row['RuCategory'];
                    $agenda =  $row['RuAgenda'];
                    $involve =  $row['RuInvolve'];
                    $type =  $row['RuType'];
                    $fundtype =  $row['RuFundType'];
                    $fundamount =  $row['RuFundAmount'];
                    $fundagency =  $row['RuFundAgency'];
                    $datestart = $row['RuDateStart'];
                    $datetarget = $row['RuDateTarget'];
                    $datecomplete = $row['RuDateCompleted'];
                    $agency = $row['RuAgency'];
                    $desc = $row['RuDesc'];
                    $level = $row['RuLevel'];
                    $file = $row['RuTempName'];
                    $original = $row['RuFilename'];
                    $change = $row['UtiId'];
                }
            }
                ...
                <div class="col-md-8">
                    <table class="table table-bordered" id="dynamic_field">
                        <tr>
                            ...
                            { 
                                $c=0;
                                $fecthname = $con->query("SELECT RuName FROM ru_name WHERE UtiId = '$record'");
                                while($var = $fecthname->fetch_assoc())
                                {  
                                    $researchersname = $var['RuName'];
                                    if($c==0)
                                    {
                                        echo "  <td><input value = '".$researchersname."' type='text' name='name[]' placeholder='Enter Name' class='form-control name_list' minlength='5' maxlength='20' required/></td>  
                                        <td><button type='button' name='add1' id='add1' class='btn btn-success'>Add More</button></td>";
                                    }
                                    else
                                    {   

                                        echo '<tr id="row'.$c.'"><td><input type="text" value="'.$researchersname.'" name="name[]" placeholder="Enter Name" class="form-control name_list" minlength="5" maxlength="20" /></td><td><button type="button" name="remove" id='.$c.' class="btn btn-danger btn_remove">X</button></td></tr>';
                                    }                    
                                    $c++;
                                }
                            }
        ...
        <div class="form-group">
            <div class="col-md-11"style="margin-bottom:15px;">
                ...
                <div class="col-md-8">
                    <select class="form-control input-sm" name="involve" id="Involvementeditable1" minlength='3' maxlength='80' value="<?php echo $involve; ?>" required>
                        <option value=""></option>
            ...
            <div class="col-md-11">
                ...
                <div class="col-md-8" style="margin-bottom:15px;">	
                    <?php
                        ...
                        if(mysqli_num_rows($query_run)>0)
                        {          
                            $fecthwords = $con->query("SELECT RuKeyword FROM ru_keywords WHERE UtiId = '$record'");
                            while($rows = $fecthwords->fetch_assoc())
                            {  
                                $words = $rows['RuKeyword'];
        ...
        <div class="form-group">
            <div class="col-md-11"style="margin-bottom:15px;">
                <label class="col-md-4 control-label" >Agency/Organization that utilized the research output:</label>
                <div class="col-md-8">
        ...
        <div class="form-group">
            <div class="col-md-11"style="margin-bottom:15px;">
                <label class="col-md-4 control-label">Brief Description of Research Utilization:</label>
                <div class="col-md-8">
        ...
        <div class="form-group">
            <div class="col-md-11"style="margin-bottom:15px;">
                <label class="col-md-4 control-label" for="Status">Level of Utilization:</label>
                <div class="col-md-8">
