...
$resul = $con->query("SELECT AoId FROM awardorg WHERE SubmissionId = (SELECT SubmissionId FROM submission WHERE UserId = '$test' AND (SubmissionStatus = 'Ongoing' OR SubmissionStatus = 'Submitted') AND QuarterId = (SELECT QuarterId FROM quarter WHERE QuarterProgress='Ongoing')) ORDER BY AoId DESC");
$resul2 = $con->query("SELECT AoId FROM awardorg WHERE SubmissionId = (SELECT SubmissionId FROM submission WHERE UserId = '$test' AND (SubmissionStatus = 'Ongoing' OR SubmissionStatus = 'Submitted') AND QuarterId = (SELECT QuarterId FROM quarter WHERE QuarterProgress='Ongoing'))");
...
<section id="part-banner">
    <div class="container">
        <div class="row">
            <div class="col-lg-12">
                <h3 class="pageTitle">E. Awards/ Recognitions Received by Office from Reputable Organization</h3>
            </div>
        </div>
    </div>
</section>
...
<div class="Form-Group1">
                     ...
                     $recordcounter = mysqli_num_rows($resul);
                            ...
                            else
                            {
                                ...
                                else
                                {
                                    if($addstat == 1)
                                    {
                                        $pick = "SELECT AoId FROM awardorg ORDER BY AoId DESC LIMIT 1";
    ...
    <form name="actorInsert" onSubmit="return ValidateActInsert()" action="Insert/AwardOrgInsert.php" method="POST" enctype="multipart/form-data">
        <?php
            ...
            {
                $queryr = "SELECT AoId FROM awardorg ORDER BY AoId DESC";
            ...
            $query = "SELECT * FROM awardorg WHERE AoId = '$record'";
        ...
        <div class="form-group">
            <div class="col-md-11"style="margin-bottom:15px;">
                <label class="col-md-4 control-label">Name of Award:</label>
                <div class="col-md-8">
                    <input name="change" type="hidden" value="<?php echo $change; ?>">
                    <input name="aname" class="form-control input-sm" placeholder="" type="text" minlength="3" maxlength="100" value="<?php echo $aname; ?>"  onkeyup="javascript:capitalize(this.id, this.value);" autocomplete="off" required>
                </div>
            </div>
        </div>
        <div class="form-group">
            <div class="col-md-11"style="margin-bottom:15px;">
                <label class="col-md-4 control-label">Certifying Body:</label>
                <div class="col-md-8">
                    <input name="body" class="form-control input-sm" placeholder="" minlength="3" maxlength="100" type="text" value="<?php echo $body; ?>"  onkeyup="javascript:capitalize(this.id, this.value);" autocomplete="off" required>
                </div>
            </div>
        </div>
        <div class="form-group">
            <div class="col-md-11"style="margin-bottom:15px;">
                ...
                <div class="col-md-8">
                    <input name="place" class="form-control input-sm" placeholder="" minlength="3" maxlength="100" type="text" value="<?php echo $place; ?>"  onkeyup="javascript:capitalize(this.id, this.value);" autocomplete="off" required>
                </div>
            </div>
        </div>
        <div class="form-group">
...
<script>
    ...
    if (document.actorInsert.aname.value.match(specialChars)) {
        alert ("Special Characters are not allowed")
        return false;
    }
    else if (document.actorInsert.body.value.match(specialChars)) {
        alert ("Special Characters are not allowed")
        return false;
    }
