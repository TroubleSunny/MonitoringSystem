...
$resul = $con->query("SELECT OpId FROM opcr WHERE SubmissionId = (SELECT SubmissionId FROM submission WHERE UserId = '$test' AND (SubmissionStatus = 'Ongoing' OR SubmissionStatus = 'Submitted') AND QuarterId = (SELECT QuarterId FROM quarter WHERE QuarterProgress='Ongoing')) ORDER BY OpId DESC");
$resul2 = $con->query("SELECT OpId FROM opcr WHERE SubmissionId = (SELECT SubmissionId FROM submission WHERE UserId = '$test' AND (SubmissionStatus = 'Ongoing' OR SubmissionStatus = 'Submitted') AND QuarterId = (SELECT QuarterId FROM quarter WHERE QuarterProgress='Ongoing'))");
...
<div class="Form-Group1">
                    ...
                    $recordcounter = mysqli_num_rows($resul);
                            ...
                            else
                            {
                                ...
                                else
                                {
                                    if($addstat == 1)
                                    {
                                        $pick = "SELECT OpId FROM opcr ORDER BY OpId DESC LIMIT 1";
    ...
    <form name="actorInsert" onSubmit="return ValidateActInsert()" action="Insert/OPCR1Insert.php" method="POST" enctype="multipart/form-data">
        <?php
            $output = null;
            $datefrom = null;
            $dateto = null;
            ...
            {
                $queryr = "SELECT OpId FROM opcr ORDER BY OpId DESC";
            ...
            $query = "SELECT * FROM opcr WHERE OpId = '$record'";
            ...
            if(mysqli_num_rows($query_run)>0)
            {
                ...
                {
                    $output = $row['OpOutput'];
                    $datefrom = $row['OpTargetDate'];
                    $dateto = $row['OpActualDate'];
                    $desc = $row['OpDesc'];
                    $status = $row['OpStatus'];
                    $remarks = $row['OpRemarks'];
                    $file = $row['OpTempName'];
                    $original = $row['OpFilename'];
                    $change = $row['OpId'];
                }
            }
        ...
        <div class="form-group">
            <div class="rows">
                <div class="col-md-11"> 
                    <label class="col-md-4 control-label" for="FNAME">Actual Date of Completion:</label>
                    <div class="col-md-8">
        ...
        <div class="form-group">
            <div class="col-md-11">
                ...
                <div class="col-md-8">
                    <input name="remarks" class="form-control input-sm" placeholder="" type="text" value="<?php echo $remarks;?>" minlength="3" maxlength="80" onkeyup="javascript:capitalize(this.id, this.value);" autocomplete="off">
                </div>
            </div>  
        </div>   
        <?php
