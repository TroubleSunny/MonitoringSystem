...
$resul = $con->query("SELECT EcId FROM eservice_consultant WHERE SubmissionId = (SELECT SubmissionId FROM submission WHERE UserId = '$test' AND (SubmissionStatus = 'Ongoing' OR SubmissionStatus = 'Submitted') AND QuarterId = (SELECT QuarterId FROM quarter WHERE QuarterProgress='Ongoing')) ORDER BY EcId DESC");
$resul2 = $con->query("SELECT EcId FROM eservice_consultant WHERE SubmissionId = (SELECT SubmissionId FROM submission WHERE UserId = '$test' AND (SubmissionStatus = 'Ongoing' OR SubmissionStatus = 'Submitted') AND QuarterId = (SELECT QuarterId FROM quarter WHERE QuarterProgress='Ongoing'))");
...
<div class="Form-Group1">
                    ...
                    $recordcounter = mysqli_num_rows($resul);
                            ...
                            else
                            {
                                ...
                                else
                                {
                                    if($addstat == 1)
                                    {
                                        $pick = "SELECT EcId FROM eservice_consultant ORDER BY EcId DESC LIMIT 1";
    ...
    <form name="actorInsert" onSubmit="return ValidateActInsert()" action="Insert/ExtensionRenderedInsert.php" method="POST" enctype="multipart/form-data">
        <?php
            ...
            {
                $queryr = "SELECT EcId FROM eservice_consultant ORDER BY EcId DESC";
            ...
            $query = "SELECT * FROM eservice_consultant WHERE EcId = '$record'";
            ...
            if(mysqli_num_rows($query_run)>0)
            {
                ...
                {
                    $class = $row['EConsultantClass'];
                    $title = $row['EConsultantTitle'];
                    $category = $row['EConsultantCategory'];
                    $agency = $row['EConsultantAgency'];
                    $venue = $row['EConsultantVenue'];
                    $level = $row['EConsultantLevel'];

                    $datefrom = $row['EConsultantDateFrom'];
                    $dateto = $row['EConsultantDateTo'];

                    $file = $row['ETempName'];
                    $original = $row['EFilename'];
                    $change = $row['EcId'];
                }
            }
        ...
        <div class="form-group">
            <div class="col-md-11"style="margin-bottom:15px;">
                <label class="col-md-4 control-label" for="FNAME">Classification of expert services rendered as a  consultant /expert:</label>
                <div class="col-md-8">
                    <input name="change" type="hidden" value="<?php echo $change; ?>">
                    <select name="class" class="form-control input-sm" id="Classificationeditable1" value="<?php echo $class;?>" minlength='3' maxlength='100' required>
                        <option value=""></option>
                        <option value="Education" >Education</option>
                        <option value="Technology" >Technology</option>
                        <option value="Arts" >Arts & Sports</option>
                        <option value="Peofessional" >Professional/Scientific</option>
                        <option value="Organizational" >Organizational Development/Management</option>
                    </select> 
                </div>
            </div>
        </div>
        <div class="form-group">
            <div class="col-md-11" style="margin-bottom:15px;">
                <label class="col-md-4 control-label" for="FNAME">Title of Expert Services Rendered:</label>
                <div class="col-md-8">
        ...
        <div class="form-group">
            <div class="col-md-11" style="margin-bottom:15px;">
                <label class="col-md-4 control-label" for="FNAME">Category of Expert Services:</label>
                <div class="col-md-8">
                    <input name="category" class="form-control input-sm" placeholder="" type="text" minlength='3' maxlength='100' value="<?php echo $category;?>"  onkeyup="javascript:capitalize(this.id, this.value);" autocomplete="off" required>
                </div>
            </div>
        </div>
        <div class="form-group">
            <div class="col-md-11" style="margin-bottom:15px;">
                <label class="col-md-4 control-label" for="FNAME">Partner Agency:</label>
                <div class="col-md-8">
                    <input name="agency" class="form-control input-sm" placeholder="" type="text" minlength='3' maxlength='100' value="<?php echo $agency;?>"  onkeyup="javascript:capitalize(this.id, this.value);" autocomplete="off" required>
                </div>
            </div>
        </div>
        <div class="form-group">
        ...
        <div class="form-group">
            <div class="col-md-11"style="margin-bottom:15px;">
                ...
                <div class="col-md-8">
                    <select class="form-control input-sm" name="level" id="Leveleditable1" value="<?php echo $level;?>" minlength='3' maxlength='100' required>
                        <option value=""></option>
...
<script>
    $(document).ready(function(){
        ...
        $('#add').click(function(){
            i++;
            $('#dynamic_field').append('<tr id="row'+i+'"><td><input type="text" name="name[]" placeholder="Enter Name" class="form-control name_list" minlength="5" maxlength="80" /></td><td><button type="button" name="remove" id="'+i+'" class="btn btn-danger btn_remove">X</button></td></tr>');  
        });
