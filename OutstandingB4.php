...
$resul = $con->query("SELECT AtId FROM attendancet WHERE SubmissionId = (SELECT SubmissionId FROM submission WHERE UserId = '$test' AND (SubmissionStatus = 'Ongoing' OR SubmissionStatus = 'Submitted') AND QuarterId = (SELECT QuarterId FROM quarter WHERE QuarterProgress='Ongoing')) ORDER BY AtId DESC");
$resul2 = $con->query("SELECT AtId FROM attendancet WHERE SubmissionId = (SELECT SubmissionId FROM submission WHERE UserId = '$test' AND (SubmissionStatus = 'Ongoing' OR SubmissionStatus = 'Submitted') AND QuarterId = (SELECT QuarterId FROM quarter WHERE QuarterProgress='Ongoing'))");
...
<section id="part-banner-specific">
    <div class="container">
        <div class="row">
            <div class="col-lg-12">
                <h4 class="pageTitle">B.3.2. Attendance in Training/s</h4>
            </div>
        </div>
    </div>
</section>
...
<div class="Form-Group1">
                    ...
                    $recordcounter = mysqli_num_rows($resul);
                            ...
                            else
                            {
                                ...
                                else
                                {
                                    if($addstat == 1)
                                    {
                                        $pick = "SELECT AtId FROM attendancet ORDER BY AtId DESC LIMIT 1";
    ...
    <form name="actorInsert" onSubmit="return ValidateActInsert()" action="Insert/AttendancetInsert.php" method="POST" enctype="multipart/form-data">
        <?php
            $atitle = null;
            $classification = null;
            $nature = null;
            ...
            {
                $queryr = "SELECT AtId FROM attendancet ORDER BY AtId DESC";
            ...
            $query = "SELECT * FROM attendancet WHERE AtId = '$record'";
            ...
            if(mysqli_num_rows($query_run)>0)
            {
                ...
                {
                    $atitle = $row['AtTitle'];
                    $classification = $row['AtClass'];
                    $nature = $row['AtNature'];
                    $budget = $row['AtBudget'];
                    $source = $row['AtSource'];  
                    $organizer = $row['AtOrganizer'];
                    $level = $row['AtLevel'];
                    $venue = $row['AtVenue'];
                    $datefrom = $row['AtDateFrom'];
                    $dateto = $row['AtDateTo'];
                    $hours = $row['AtHours'];
                    $file = $row['AtTempName'];
                    $original = $row['AtFilename'];
                    $change = $row['AtId'];
                }
            }
        ...
        <div class="form-group">
            <div class="col-md-11">
                ...
                <div class="col-md-8">
                    ...
                    <input name="title" class="form-control input-sm" placeholder="" type="text" value="<?php echo $atitle; ?>"  onkeyup="javascript:capitalize(this.id, this.value);" minlength="3" maxlength="80" autocomplete="off" required>
                </div>
            </div>
        </div>   
        <div class="form-group">
            <div class="col-md-11">
                ...
                <div class="col-md-8">
                    <select class="form-control input-sm" name="classification" id="Classificationeditable" value="<?php echo $classification; ?>" minlength="3" maxlength="80" required>
                        ...
                        <option value="Professional">Professional/Continuing Professional Development</option>
                        <option value="Arts/Media/Culture & Sports">Short Term Courses</option>
                        <option value="Invention">Executive/Manegerial</option>   
                    </select> 
                </div>
            </div>
        </div> 
        ...
        <div class="form-group">
            <div class="col-md-11">
                ...
                <div class="col-md-8">
                    <input name="budget" class="form-control input-sm" placeholder="" type="number" step="0.01" value="<?php echo $budget; ?>" onkeyup="javascript:capitalize(this.id, this.value);" minlength="5" maxlength="80" autocomplete="off" required>
                </div>
            </div>
        </div> 
        <div class="form-group">
        ...
        <div class="form-group">
            <div class="col-md-11">
                ...
                <div class="col-md-8">
                    <input name="organizer" class="form-control input-sm" placeholder="" type="text" value="<?php echo $organizer; ?>" onkeyup="javascript:capitalize(this.id, this.value);" autocomplete="off" minlength="3" maxlength="80" required>
                </div>
            </div>
        </div> 
        <div class="form-group">
        ...
        <div class="form-group">
            <div class="col-md-11">
                ...
                <div class="col-md-8">
                    <input name="hours" class="form-control input-sm" placeholder="" type="number" value="<?php echo $hours; ?>" onkeyup="javascript:capitalize(this.id, this.value);" autocomplete="off">
                </div>
            </div>
        </div> 
        <?php
...
<script type="text/javascript">
    document.getElementById("skipbtn").onclick = function () {
        <?php
            ...
            mysqli_query($con,$stat);
        ?>
        location.href = "AccomplishmentOPCR1.php";
    };
</script>
<script type="text/javascript">
...
<script>
    ...
    else if (document.actorInsert.organizer.value.match(specialChars)) {
        alert ("Special Characters are not allowed")
        return false;
    }
    return (true);
