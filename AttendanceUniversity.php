...
$resul = $con->query("SELECT AuId FROM attendanceu WHERE SubmissionId = (SELECT SubmissionId FROM submission WHERE UserId = '$test' AND (SubmissionStatus = 'Ongoing' OR SubmissionStatus = 'Submitted') AND QuarterId = (SELECT QuarterId FROM quarter WHERE QuarterProgress='Ongoing')) ORDER BY AuId DESC");
$resul2 = $con->query("SELECT AuId FROM attendanceu WHERE SubmissionId = (SELECT SubmissionId FROM submission WHERE UserId = '$test' AND (SubmissionStatus = 'Ongoing' OR SubmissionStatus = 'Submitted') AND QuarterId = (SELECT QuarterId FROM quarter WHERE QuarterProgress='Ongoing')) ");
...
<section id="Form-banner">
    <div class="container">
        <div class="row">
            <div class="col-lg-12">
                <h2 class="pageTitle">III. Attendance in University Function</h2>
            </div>
        </div>
    </div>
</section>
...
<div class="Form-Group1">
                    ...
                    $recordcounter = mysqli_num_rows($resul);
                            ...
                            else
                            {
                                ...
                                else
                                {
                                    if($addstat == 1)
                                    {
                                        $pick = "SELECT AuId FROM attendanceu ORDER BY AuId DESC LIMIT 1";
    ...
    <form name="actorInsert" onSubmit="return ValidateActInsert()" action="Insert/AttendanceUniversityInsert.php" method="POST" enctype="multipart/form-data">
        <?php
            $desc = null;
            $datefrom = null;
            $dateto = null;
            ...
            {
                $queryr = "SELECT AuId FROM attendanceu ORDER BY AuId DESC";
            ...
            $query = "SELECT * FROM attendanceu WHERE AuId = '$record'";
            ...
            if(mysqli_num_rows($query_run)>0)
            {
                ...
                {
                    $desc = $row['AuDesc'];
                    $datefrom = $row['AuDateStart'];
                    $dateto = $row['AuDateComp'];
                    $status = $row['AuStatus'];
                    $file = $row['AuTempName'];
                    $original = $row['AuFilename'];
                    $change = $row['AuId'];
                }
            }
        ...
        <div class="form-group">
            <div class="col-md-11">
                <label class="col-md-4 control-label" for="FNAME">Brief Description of Activity:</label>
                <div class="col-md-8">
        ...
        <div class="form-group">
            <div class="rows">
                <div class="col-md-11">                
                    <label class="col-md-4 control-label" for="FNAME">Date Started:</label>
                    <div class="col-md-8">
        ...
        <div class="form-group">
            <div class="col-md-11">
                <label class="col-md-4 control-label" for="FNAME">Status of Attendance:</label>
                <div class="col-md-8">
                    <input name="status" class="form-control input-sm" placeholder="" type="text" value="<?php echo $status;?>" minlength="3" maxlength="80" onkeyup="javascript:capitalize(this.id, this.value);" autocomplete="off" required>
                </div>
            </div>
        </div>
        <?php
...
<script type="text/javascript">
    document.getElementById("backbtn").onclick = function () {
        <?php
            ...
            mysqli_query($con,$stat);
        ?>
        location.href = "AccomplishmentOPCR3.php";
    };
</script>
