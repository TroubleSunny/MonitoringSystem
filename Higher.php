...
while($row = mysqli_fetch_assoc($yearresult))
{
    ...
    $complete_query = ("SELECT RoId FROM researchongoing WHERE RoStatus = 'Completed' AND SubmissionId IN (SELECT SubmissionId FROM submission WHERE ValidationStatus = 'Accepted' AND QuarterId IN (SELECT QuarterId FROM quarter WHERE YearId IN (SELECT YearId FROM schoolyear WHERE SchoolYear = '$school')))");
    $complete_result = mysqli_query($con,$complete_query);
    $complete_count = mysqli_num_rows($complete_result);

    $ongoing_query = ("SELECT RoId FROM researchongoing WHERE RoStatus = 'Ongoing' AND SubmissionId IN (SELECT SubmissionId FROM submission WHERE ValidationStatus = 'Accepted' AND QuarterId IN (SELECT QuarterId FROM quarter WHERE YearId IN (SELECT YearId FROM schoolyear WHERE SchoolYear = '$school')))");
    $ongoing_result = mysqli_query($con,$ongoing_query);
    $ongoing_count = mysqli_num_rows($ongoing_result);

    $deffered_query = ("SELECT RoId FROM researchongoing WHERE RoStatus = 'Deferred' AND SubmissionId IN (SELECT SubmissionId FROM submission WHERE ValidationStatus = 'Accepted' AND QuarterId IN (SELECT QuarterId FROM quarter WHERE YearId IN (SELECT YearId FROM schoolyear WHERE SchoolYear = '$school')))");
    $deferred_result = mysqli_query($con,$deffered_query);
    $deferred_count = mysqli_num_rows($deferred_result);

    $publication_query = ("SELECT RpId FROM researchpublication WHERE SubmissionId IN (SELECT SubmissionId FROM submission WHERE ValidationStatus = 'Accepted' AND QuarterId IN (SELECT QuarterId FROM quarter WHERE YearId IN (SELECT YearId FROM schoolyear WHERE SchoolYear = '$school')))");
    $publication_result = mysqli_query($con,$publication_query);
    $publication_count = mysqli_num_rows($publication_result);

    $presentation_query = ("SELECT RpresId FROM researchpresentation WHERE SubmissionId IN (SELECT SubmissionId FROM submission WHERE ValidationStatus = 'Accepted' AND QuarterId IN (SELECT QuarterId FROM quarter WHERE YearId IN (SELECT YearId FROM schoolyear WHERE SchoolYear = '$school')))");
    $presentation_result = mysqli_query($con,$presentation_query);
    $presentation_count = mysqli_num_rows($presentation_result);

    $citation_query = ("SELECT CitationId FROM researchcitation WHERE SubmissionId IN (SELECT SubmissionId FROM submission WHERE ValidationStatus = 'Accepted' AND QuarterId IN (SELECT QuarterId FROM quarter WHERE YearId IN (SELECT YearId FROM schoolyear WHERE SchoolYear = '$school')))");
    $citation_result = mysqli_query($con,$citation_query);
    $citation_count = mysqli_num_rows($citation_result);

    $citation_scopus_query = ("SELECT CitationId FROM researchcitation WHERE RcJournalIndexing = 'Scopus' AND SubmissionId IN (SELECT SubmissionId FROM submission WHERE ValidationStatus = 'Accepted' AND QuarterId IN (SELECT QuarterId FROM quarter WHERE YearId IN (SELECT YearId FROM schoolyear WHERE SchoolYear = '$school')))");
    ...
    $utilization_query = ("SELECT UtiId FROM researchutilization WHERE SubmissionId IN (SELECT SubmissionId FROM submission WHERE ValidationStatus = 'Accepted' AND QuarterId IN (SELECT QuarterId FROM quarter WHERE YearId IN (SELECT YearId FROM schoolyear WHERE SchoolYear = '$school')))");
    $utilization_result = mysqli_query($con,$utilization_query);
    $utilization_count = mysqli_num_rows($utilization_result);

    $copyright_query = ("SELECT CoId FROM copyrightedoutput WHERE SubmissionId IN (SELECT SubmissionId FROM submission WHERE ValidationStatus = 'Accepted' AND QuarterId IN (SELECT QuarterId FROM quarter WHERE YearId IN (SELECT YearId FROM schoolyear WHERE SchoolYear = '$school')))");
...
<body class="main-level">
    ...
    <div class="wrapper">
        ...
        <div class="sidebar" id="sidebar">
            ...
            <ul>
                <li><a class="choice" href=# onclick="openTab('Submissions')"><i class="fas fa-scroll"></i><span>Submissions</span></a></li>

                <li><a class="choice" href=# onclick="openTab('Members')"><i class="fas fa-user-friends"></i><span>Faculty members</span></a></li>
                <li id="ResearchTab"><a class="choice" href="#Research" onclick="openTab('Research')"><i class="fas fa-search"></i><span>University Research</span></a></li>
    ...
    <div id="Submissions" class="Contents tab">
        <div class="stuff-banner">
            <section id="inner-headline">
                <div class="container">
                    <div class="row">
                        <div class="col-lg-12">
                            <h2 class="pageTitle" style="font-size:28px; margin-top:0;">University QAR Submissions <?php echo "$displayquarter ($displayyear)";  ?></h2>
        ...
        <div class="input-group rounded"style="margin-bottom:20px; ">
            ...
            <label class="labeler" for="Status" style="margin-left:60px;">Sort By College:</label>            
            <select class="form-controler" name="collegeval" id="collegeval">
                <?php

                        $college = "SELECT College FROM college ORDER BY CollegeId DESC";
                        ...
                        if(mysqli_num_rows($collegeval)>0)
                        {
                            ...
                            {
                                $collegepart = $collegepart['College'];
                ...
                <table class="table table-main">
                    <thead>
                        <tr>
                            ...
                            <th style="text-align:center;">View Submission</th>
                        </tr>
                    </thead>
                    <tbody>
                        <?php

                        $query = "SELECT * FROM submission WHERE ValidationStatus = 'Accepted' AND QuarterId IN (SELECT QuarterId FROM quarter WHERE QuarterProgress = 'Ongoing') ";
            ...
            <div class="containerz1" id="modalz_containerz1">
                <div class="row">
                    <div class="moz1">
                        <div class="subcon1">
                            <table class="table-main">
                                ...
                                <thead>
                                    <tr>
                                        <th scope="col" span="1" style="width:12px;">#</th>
                                ...
                                <tbody>
                                    ...
                                    $query = "SELECT * FROM submission WHERE ValidationStatus = 'Accepted'";
                                    $r = mysqli_query($con,$query);
                                    $i = 1;
                                    while($row = mysqli_fetch_assoc($r)){
                                    ...
                                    <tr>
                                        ...
                                        <form action="" method="post">
                                            <th style="text-align:center; ">
                                                <input type="submit" name="remove" class="btn btn-warning" value="Send Feedback">
                                            </th>
                                        </form>
                                    </tr>
                                    <?php
                        ...
                        <div class="Formbtn1">
                            <button class="btn btn-success" id="printbtn1">Download QAR</button>
                            <button class="btn btn-warning" id="closez1">Close</button>
                        </div>
                    </div>
                </div>
            </div>
    ...
    <div id="Members" class="Contents tab" style="display:none;">
        <div class="stuff-banner">
            <section id="inner-headline">
                <div class="container">
                    <div class="row">
                        <div class="col-lg-12">
                            <h2 class="pageTitle" style="font-size:35px; " >University Faculty Members</h2>
                        </div>
                    </div>
                </div>
            </section>
        ...
        <div class="input-group rounded"style="margin-bottom:20px; ">
            ...
            <select class="form-controler" name="collegeval2" id="collegeval2">
                <?php
            ...
            <table class="table-main">
                ...
                <tbody>
                    ...
                    $query = "SELECT * FROM user ORDER BY UserLevel ";
                    $r = mysqli_query($con,$query);
                    $i = 1;
                    while($row = mysqli_fetch_assoc($r)){
                        $userid=$row['UserId'];
                    ...
                    <div class="moz">
                        <div class="col-sm-11">
                            ...
                            <select class="form-control" name="userlevel" id="userlevel" required>
                                ...
                                <option value="International" >Dean</option>
                                <option value="National">Faculty Member</option>
                                <option value="National">Higher</option>

                            </select> 
                            <hr class ="mb-3">
                            <div class="Formbtn">
                                <button class="btn btn-success" id="addmember">Add</button>
                                <button class="btn btn-warning" id="closez">Close</button>
                            </div>
                        </div>
                    </div>
                </div>
    ...
    <div id="Research" class="Contents tab" style="display:none;">
        <div class="stuff-banner">
            <section id="inner-headline">
                <div class="container">
                    <div class="row">
                        <div class="col-lg-12">
                            <h2 class="pageTitle" style="font-size:35px; " >University Research Titles</h2>
        ...
        <div class="stuff">
            ...
            <div id="All" class="Contents tab2" role="tabpanel">
                <div class="input-group rounded"style="margin-bottom:20px; ">


                    <label class="labeler" for="Status" style="margin-left:60px;">Sort By College:</label>
                    
                    <select class="form-controler" name="collegevalAll" id="collegevalAll">
                        <?php
                ...
                <div class="tableAll">
                    <table class="table-main" style="display:block; height:400px; overflow: auto;">
                        ...
                        <tbody>
                            <?php 
                                $query = "SELECT * FROM researchongoing ORDER BY RoDateStart DESC";
                        ...
                        ?>
                            ...
                            <?php 
                                $query = "SELECT * FROM researchpresentation ORDER BY RpresDateStart DESC";
                                $query_run = mysqli_query($con,$query);
                        ...
                        ?>
                            ...
                            <?php 
                                $query = "SELECT * FROM researchpublication";
                                $query_run = mysqli_query($con,$query);
                        ...
                        ?>
                            ...
                            <?php 
                                $query = "SELECT * FROM researchcitation ORDER BY RcDateStart";
                                $query_run = mysqli_query($con,$query);
                        ...
                        ?>
                            ...
                            <?php 
                                $query = "SELECT * FROM researchutilization ORDER BY RuDateStart";
                                $query_run = mysqli_query($con,$query);
                        ...
                        ?>
                            ...
                            <?php 
                                $query = "SELECT * FROM copyrightedoutput ORDER BY CoDateStart";
                                $query_run = mysqli_query($con,$query);
            ...
            <div id="Ongoing" class="Contents tab2" style="display:none;" role="tabpanel">
                <div class="input-group rounded" style="margin-bottom:20px; ">

                    <label class="labeler" for="Status" style="margin-left:60px;">Sort By College:</label>
                    
                    <select class="form-controler" name="collegevalOngoing" id="collegevalOngoing">
                        <?php
                ...
                <div class="tableOngoing">
                    <table class="table-main" style="display:block; height:400px; overflow: auto;">
                        ...
                        <tbody>
                            <?php 
                                $query = "SELECT * FROM researchongoing WHERE RoStatus='Ongoing' AND SubmissionId IN(SELECT SubmissionId FROM submission WHERE ValidationStatus = 'Accepted') ORDER BY RoDateStart DESC";
                                $query_run = mysqli_query($con,$query);
                                $t = 1;
                                if(mysqli_num_rows($query_run)>0)
                                {
            ...
            <div id="Completed" class="Contents tab2" style="display:none;">
                <div class="input-group rounded"style="margin-bottom:20px; ">
                    ...
                    <select class="form-controler" name="collegevalCompleted" id="collegevalCompleted">
                        <?php
                ...
                <div class="tableCompleted">
                    <table class="table-main" style="display:block; height:400px; overflow: auto;">
                        ...
                        <tbody>
                            <?php 
                                $query = "SELECT * FROM researchongoing WHERE RoStatus = 'Completed' AND SubmissionId IN(SELECT SubmissionId FROM submission WHERE ValidationStatus = 'Accepted') ORDER BY RoDateStart DESC";
                                $query_run = mysqli_query($con,$query);
                                $t = 1;
                                if(mysqli_num_rows($query_run)>0)
                                {
                ...
                <button id="save" class="btn btn-primarys">Download</button>
            </div>
            <div id="Unfinished" class="Contents tab2" style="display:none;">
                <div class="input-group rounded"style="margin-bottom:20px; ">
                    ...
                    <select class="form-controler" name="collegevalUnfinished" id="collegevalUnfinished">
                        <?php
            ...
            <div id="Publication" class="Contents tab2" style="display:none;">
                <div class="input-group rounded"style="margin-bottom:20px; ">
                    ...
                    <select class="form-controler" name="collegevalPublication" id="collegevalPublication">
                        <?php
                ...
                <div class="tablePublished">
                    <table class="table-main" style="display:block; height:400px; overflow: auto;">
                        ...
                        <tbody>
                                ...
                                <?php 
                                    $query = "SELECT * FROM researchpublication WHERE SubmissionId IN(SELECT SubmissionId FROM submission WHERE ValidationStatus = 'Accepted') ORDER BY RpDateStart DESC";
                                    $query_run = mysqli_query($con,$query);
                                    $t=1;
                                    if(mysqli_num_rows($query_run)>0)
                                    {
            ...
            <div id="Presentation" class="Contents tab2" style="display:none;">
                <div class="input-group rounded"style="margin-bottom:20px; ">
                    ...
                    <select class="form-controler" name="collegevalPresentation" id="collegevalPresentation">
                        <?php
                ...
                <div class="tablePresentation">
                    <table class="table-main" style="display:block; height:400px; overflow: auto;">
                        ...
                        <tbody>
                                ...
                                <?php 
                                    $query = "SELECT * FROM researchpresentation WHERE SubmissionId IN(SELECT SubmissionId FROM submission WHERE ValidationStatus = 'Accepted') ORDER BY RpresDateStart DESC";
                                    $query_run = mysqli_query($con,$query);
                                    $t=1;
                                    if(mysqli_num_rows($query_run)>0)
                                    {
            ...
            <div id="Citation" class="Contents tab2" style="display:none;">
                <div class="input-group rounded"style="margin-bottom:20px; ">
                    ...
                    <select class="form-controler" name="collegevalCitation" id="collegevalCitation">
                        <?php
                ...
                <div class="tableCitation">
                    <table class="table-main" style="display:block; height:400px; overflow: auto;">
                        ...
                        <tbody>
                        <?php 
                                $query = "SELECT * FROM researchcitation WHERE SubmissionId IN(SELECT SubmissionId FROM submission WHERE ValidationStatus = 'Accepted') ORDER BY RcDateStart DESC";
                                $query_run = mysqli_query($con,$query);
                                $t=1;
                                if(mysqli_num_rows($query_run)>0)
                                {
            ...
            <div id="Utilization" class="Contents tab2" style="display:none;">
                <div class="input-group rounded"style="margin-bottom:20px; ">
                    ...
                    <select class="form-controler" name="collegevalUtilization" id="collegevalUtilization">
                        <?php
                ...
                <div class="tableUtilization">
                    <table class="table-main" style="display:block; height:400px; overflow: auto;">
                        ...
                        <tbody>
                            <?php 
                                $query = "SELECT * FROM researchutilization WHERE SubmissionId IN(SELECT SubmissionId FROM submission WHERE ValidationStatus = 'Accepted') ORDER BY RuDateStart DESC";
                                $query_run = mysqli_query($con,$query);
                                $t=1;
                                if(mysqli_num_rows($query_run)>0)
                                {
            ...
            <div id="Copyright" class="Contents tab2" style="display:none;">
                <div class="input-group rounded"style="margin-bottom:20px; ">
                    ...
                    <select class="form-controler" name="collegevalCopyright" id="collegevalCopyright">
                        <?php
                ...
                <div class="tableCopyright">
                    <table class="table-main" style="display:block; height:400px; overflow: auto;">
                        ...
                        <tbody>
                            <?php 
                                $query = "SELECT * FROM copyrightedoutput WHERE SubmissionId IN(SELECT SubmissionId FROM submission WHERE ValidationStatus = 'Accepted') ORDER BY CoDateStart DESC";
                                $query_run = mysqli_query($con,$query);
                ...
                <div id="div_copyright">
                    <h2 align="center">PUP Copyrighted Research Output</h2>
    ...
    <script>
        $(document).ready(function(){
            $('#printsubmittedqar').click(function(){
                var value = $('#levelval').val();
                var col = $('#collegeval').val();
                
                $.ajax({
                    url:"DisplayAllQAR.php",
                    type:"POST",
                    data:{request: value, col: col},
    ...
    <script>
        $(document).ready(function(){
            $('#printresearchbtn').click(function(){
                var value = $('#levelvalAll').val();
                var col = $('#collegevalAll').val();
                
                $.ajax({
                    url:"printResearchHigher.php",
                    type:"POST",
                    data:{request: value, col: col},
                    success:function(data){
                        window.open('printResearchHigher.php?col='+col);
                    }
    ...
    <script>
        const addbtnz1 = document.getElementById('addbtnz1');
        const modelz_container1 = document.getElementById('modalz_containez1');
        const closez1 = document.getElementById('closez1');

        addbtnz1.addEventListener('click',()=>{
            modalz_containerz1.classList.add('show');
        });

        closez1.addEventListener('click',()=>{
            modalz_containerz1.classList.remove('show');
        });
    </script>
    ...
    <script type="text/javascript">
        $(document).ready(function(){
            $("#collegeval").on('change',function(){
                var col = $(this).val();
                var value = $('#quarterval').val();
    ...
    <script type="text/javascript">
        $(document).ready(function(){
            $("#collegevalAll").on('change',function(){
                var col = $(this).val();
                var value = $('#levelvalAll').val();
                var year = $('#yearvalAll').val();
    ...
    <script type="text/javascript">
        $(document).ready(function(){
            $("#collegevalOngoing").on('change',function(){
                var col = $(this).val();
    ...
    <script type="text/javascript">
        $(document).ready(function(){
            $("#collegevalCompleted").on('change',function(){
                var col = $(this).val();
    ...
    <script type="text/javascript">
        $(document).ready(function(){
            $("#yearvalCompleted").on('change',function(){
                var year = $(this).val();
                var col = $('#collegevalCompleted').val();
    ...
    <script type="text/javascript">
        $(document).ready(function(){
            $("#collegevalUnfinished").on('change',function(){
                var col = $(this).val();
    ...
    <script type="text/javascript">
        $(document).ready(function(){
            $("#yearvalUnfinished").on('change',function(){
                var year = $(this).val();
                var col = $('#collegevalUnfinished').val();
    ...
    <script type="text/javascript">
        $(document).ready(function(){
            $("#levelvalPublication").on('change',function(){
                var value = $(this).val();
                var col = $('#collegevalPublication').val();
    ...
    <script type="text/javascript">
        $(document).ready(function(){
            $("#collegevalPublication").on('change',function(){
                var col = $(this).val();
    ...
    <script type="text/javascript">
        $(document).ready(function(){
            $("#levelvalPresentation").on('change',function(){
                var value = $(this).val();
                var col = $('#collegevalPresentation').val();
    ...
    <script type="text/javascript">
        $(document).ready(function(){
            $("#collegevalPresentation").on('change',function(){
                var col = $(this).val();
    ...
    <script type="text/javascript">
        $(document).ready(function(){
            $("#collegevalCitation").on('change',function(){
                var col = $(this).val();
    ...
    <script type="text/javascript">
        $(document).ready(function(){
            $("#yearvalCitation").on('change',function(){
                var year = $(this).val();
                var indexing = $('#indexing').val();
                var col = $('#collegevalCitation').val();
    ...
    <script type="text/javascript">
        $(document).ready(function(){
            $("#levelvalUtilization").on('change',function(){
                var value = $(this).val();
                var col = $('#collegevalUtilization').val();
    ...
    <script type="text/javascript">
        $(document).ready(function(){
            $("#collegevalUtilization").on('change',function(){
                var col = $(this).val();
    ...
    <script type="text/javascript">
        $(document).ready(function(){
            $("#levelvalCopyright").on('change',function(){
                var value = $(this).val();
                var col = $('#collegevalCopyright').val();
    ...
    <script type="text/javascript">
        $(document).ready(function(){
            $("#collegevalCopyright").on('change',function(){
                var col = $(this).val();
    ...
    <script>
        $(document).ready(function(){
            $('.viewOngoingResearch').click(function(){
                var year = $('#yearvalOngoing').val();
                var college = $('#collegevalOngoing').val();
    ...
    <script>
        $(document).ready(function(){
            $('.viewCompletedResearch').click(function(){
                var year = $('#yearvalCompleted').val();
                var college = $('#collegevalCompleted').val();
    ...
    <script>
        $(document).ready(function(){
            $('.viewUnfinishedResearch').click(function(){
                var year = $('#yearvalUnfinished').val();
                var college = $('#collegevalUnfinished').val();
    ...
    <script>
        $(document).ready(function(){
            $('.viewPublicationResearch').click(function(){
                ...
                var year = $('#yearvalPublication').val();
                var college = $('#collegevalPublication').val();
    ...
    <script>
        $(document).ready(function(){
            $('.viewPresentationResearch').click(function(){
                ...
                var year = $('#yearvalPresentation').val();
                var college = $('#collegevalPresentation').val();
    ...
    <script>
        $(document).ready(function(){
            $('.viewCitationResearch').click(function(){
                var indexing = $('#indexing').val();
                var year = $('#yearvalCitation').val();
                var college = $('#collegevalCitation').val();
    ...
    <script>
        $(document).ready(function(){
            $('.viewUtilizationResearch').click(function(){
                ...
                var year = $('#yearvalUtilization').val();
                var college = $('#collegevalUtilization').val();
    ...
    <script>
        $(document).ready(function(){
            $('.viewCopyrightResearch').click(function(){
                ...
                var year = $('#yearvalCopyright').val();
                var college = $('#collegevalCopyright').val();
    ...
    <script>
        var chart_ongoing = new Morris.Bar({
