...
$resul = $con->query("SELECT CoId FROM copyrightedoutput WHERE SubmissionId = (SELECT SubmissionId FROM submission WHERE UserId = '$test' AND (SubmissionStatus = 'Ongoing' OR SubmissionStatus = 'Submitted') AND QuarterId = (SELECT QuarterId FROM quarter WHERE QuarterProgress='Ongoing')) ORDER BY CoId DESC");
$resul2 = $con->query("SELECT CoId FROM copyrightedoutput WHERE SubmissionId = (SELECT SubmissionId FROM submission WHERE UserId = '$test' AND (SubmissionStatus = 'Ongoing' OR SubmissionStatus = 'Submitted') AND QuarterId = (SELECT QuarterId FROM quarter WHERE QuarterProgress='Ongoing'))");
...
<section id="part-banner-specific">
    <div class="container">
        <div class="row">
            <div class="col-lg-12">
                <h4 class="pageTitle">A.6.  Copyrighted Research Output</h4>
            </div>
        </div>
    </div>
</section>
...
<div class="Form-Group1">
    <form action="" method="GET">
        <div class="form-group">
            <div class="col-md-4"style="margin:15px; float:right;">
                <label class="col-md-4 control-label" for="Status">Record ID:</label>
                <div class="col-lg-3">
                     ...
                     $recordcounter = mysqli_num_rows($resul);
                            ...
                            else
                            {
                                ...
                                else
                                {
                                    if($addstat == 1)
                                    {
                                        $pick = "SELECT CoId FROM copyrightedoutput ORDER BY CoId DESC LIMIT 1";
    ...
    <form name="actorInsert" onSubmit="return ValidateActInsert()" action="Insert/CopyrightedOutputInsert.php" method="POST" enctype="multipart/form-data">
        <?php
            $class = null;
            $category = null;
            $agenda = null;
            ...
            {
                $queryr = "SELECT CoId FROM copyrightedoutput ORDER BY CoId DESC";
            ...
            $query = "SELECT * FROM copyrightedoutput WHERE CoId = '$record'";
            ...
            if(mysqli_num_rows($query_run)>0)
            {
                foreach($query_run as $row)
                {
                    $title = $row['CoTitle'];
                    $class =  $row['CoClass'];
                    $category =  $row['CoCategory'];
                    $agenda =  $row['CoAgenda'];
                    $involve =  $row['CoInvolve'];
                    $type =  $row['CoType'];
                    $fundtype =  $row['CoFundType'];
                    $fundamount =  $row['CoFundAmount'];
                    $fundagency =  $row['CoFundAgency'];
                    $datestart = $row['CoDateStart'];
                    $datetarget = $row['CoDateTarget'];
                    $datecomplete = $row['CoDateCompleted'];
                    $copyrightnum = $row['CoCopyrightNum']; 
                    $copyrightagency = $row['CoCopyrightAgency'];
                    $copyrightyear = $row['CoCopyrightYear'];
                    $level = $row['CoLevel'];
                    $file = $row['CoTempName'];
                    $original = $row['CoFilename'];
                    $change = $row['CoId'];
                }
            }
        ...
        <div class="form-group">
            <div class="col-md-11">
                ...
                <div class="col-md-8">
                    ...
                    <select class="form-control input-sm" name="title" id="Titleeditable1" value="<?php echo $title; ?>" minlength='10' maxlength='120' required>
                        <?php
        ...
        <div class="form-group">
            <div class="col-md-11">
                <label class="col-md-4 control-label" >Researcher/s (Surname, First Name, M.I.):</label>
                <div class="col-md-8">
                    <table class="table table-bordered" id="dynamic_field">
                        <tr>
                            ...
                            { 
                                $c=0;
                                $fecthname = $con->query("SELECT CoName FROM co_name WHERE CoId = '$record'");
                                while($var = $fecthname->fetch_assoc())
                                {  
                                    $researchersname = $var['CoName'];
                                    if($c==0)
                                    {
            ...
            <div class="col-md-11">
                ...
                <div class="col-md-8" style="margin-bottom:15px;">	
                    <?php
                        ...
                        if(mysqli_num_rows($query_run)>0)
                        {          
                            $fecthwords = $con->query("SELECT CoKeywords FROM co_keywords WHERE CoId = '$record'");
                            while($rows = $fecthwords->fetch_assoc())
                            {  
                                $words = $rows['CoKeywords'];
        ...
        <div class="form-group">
            <div class="col-md-11"style="margin-bottom:15px;">
                ...
                <div class="col-md-8">
                    <input name="fundagency" class="form-control input-sm" placeholder="" type="text" minlength='3' maxlength='80' value="<?php echo $fundagency; ?>"  onkeyup="javascript:capitalize(this.id, this.value);" autocomplete="off">
                </div>
            </div>
        </div>
        <div class="form-group">
        ...
        <div class="form-group">
            <div class="col-md-11"style="margin-bottom:15px;">
                <label class="col-md-4 control-label">Copyright Number (ISSN/ISBN):</label>
                <div class="col-md-8">
                    <input name="copyrightnum" class="form-control input-sm" placeholder="" type="text" value="<?php echo $copyrightnum; ?>"  onkeyup="javascript:capitalize(this.id, this.value);" autocomplete="off">
                </div>
            </div>
        </div>
        <div class="form-group">
            <div class="col-md-11"style="margin-bottom:15px;">
                <label class="col-md-4 control-label">Copyright Agency:</label>
                <div class="col-md-8">
                    <input name="copyrightagency" class="form-control input-sm" placeholder="" minlength='3' maxlength='80' type="text" value="<?php echo $copyrightagency; ?>" onkeyup="javascript:capitalize(this.id, this.value);" autocomplete="off">
                </div>
            </div>
        </div>
        <div class="form-group">
            <div class="col-md-11"style="margin-bottom:15px;">
                <label class="col-md-4 control-label">Year the research copyrighted:</label>
                <div class="col-md-8">
                    <input name="copyrightyear" class="form-control input-sm" placeholder="" type="number" value="<?php echo $copyrightyear; ?>"  onkeyup="javascript:capitalize(this.id, this.value);" autocomplete="off">
                </div>
            </div>
        </div>
        <div class="form-group">
            <div class="col-md-11"style="margin-bottom:15px;">
                ...
                <div class="col-md-8">
                    <select class="form-control input-sm" name="level" id="Leveleditable" minlength='3' maxlength='80' value="<?php echo $level; ?>">
                        <option value=""></option>
...
<script>
    $(document).ready(function(){
        ...
        $('#add1').click(function(){
            i++;
            $('#dynamic_field').append('<tr id="row'+i+'"><td><input type="text" name="name[]" placeholder="Enter Name" class="form-control name_list" minlength="5" maxlength="20" required/></td><td><button type="button" name="remove" id="'+i+'" class="btn btn-danger btn_remove">X</button></td></tr>');  
        });
